(function($){
    ...
    $(document).on('click', '#ai-seo-rm-run', function(){
        ...
        $.post(AISEO_RM.ajaxurl, {
            ...
            if (d.analysis){
                html += '<div><strong>Resumo técnico</strong></div>';
                html += '<pre style="white-space:pre-wrap;">'+ JSON.stringify(d.analysis, null, 2) +'</pre>';
            }
            if (d.ai){
                html += '<div><strong>Sugestão da IA</strong></div>';
                html += '<pre style="white-space:pre-wrap;">'+ JSON.stringify(d.ai, null, 2) +'</pre>';
            }
            if (d.applied){
                html += '<div><strong>Aplicado</strong></div>';
                html += '<pre style="white-space:pre-wrap;">'+ JSON.stringify(d.applied, null, 2) +'</pre>';
            }
            $res.html(html);
        }).fail(function(){
            $res.html('<span style="color:#b32d2e;">Erro na chamada.</span>');
        });
    });
})(jQuery);